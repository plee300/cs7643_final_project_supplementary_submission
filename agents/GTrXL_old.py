...
## Modified version of implementation from: https://opendilab.github.io/DI-engine/_modules/ding/torch_utils/network/gtrxl.html
## based on https://opendilab.github.io/DI-engine/12_policies/gtrxl.html
...
class PositionalEmbedding(nn.Module):
    def __init__(self, embedding_dim: int):
        ...
        self.register_buffer('inv_freq', inv_freq) # no training being done on frequency transform

    def forward(self, pos_seq: torch.Tensor) -> torch.Tensor:
        """
        Args:
            pos_seq (tensor): tensor of all previous positions, 1-D tensor
        
        Returns:
            positional embeddings of shape (seq_len, 1, embedding_dim)
        """
...
class GRUGatingUnit(nn.Module):
    ...
    def forward(self, x, y):
        """
        Args:
            x (tensor): unmodified input into gtrxl
            y (tensor): attention-modified x tensor
                x and y should have the same shape and have last dim equal to self.input_dim
        Returns:
            gtrxl residual 
        """

        r = self.sig(self.wr(y) + self.ur(x))
        z = self.sig(self.wz(y) + self.uz(x) - self.bg)
        h_hat = self.tanh(self.wg(y) + self.ug(r*x))

        return (1-z)*x + z*h_hat


class Memory(nn.Module):
    # Copied 1 for 1 from https://opendilab.github.io/DI-engine/_modules/ding/torch_utils/network/gtrxl.html
    def __init__(self, input_dim: int, bg: float = 2.):
        1
...
class VisionModule(nn.Module):
    # duplicate of visual dqn module
...
class GTrXL(nn.Module):
    ...
    def forward(self, x: torch.Tensor, batch_first: bool = False, return_mem: bool = True) -> Dict[str, torch.Tensor]:
        ...
        if return_mem:
            output = (out, memory)  # return the content of the memory before the last update
        else:
            output = (out)
        return output
